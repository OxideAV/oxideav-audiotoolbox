//! AMR-NB decoder backed by macOS AudioConverter.
//!
//! Input:  one storage-format AMR-NB packet per `Packet` — a TOC byte
//!         followed by the mode-determined number of speech payload
//!         bytes (1, 6, 13, 14, 16, 18, 20, 21, 27, or 32 total bytes
//!         depending on the frame type encoded in the TOC).
//! Output: interleaved S16 PCM `AudioFrame`s at 8 kHz mono. Empirically
//!         AT's AMR-NB decoder vends PCM in **120-sample** blocks (15 ms
//!         at 8 kHz) per `FillComplexBuffer` call rather than the 160-
//!         sample AMR analysis-frame size — this is an internal AT
//!         chunking detail and the per-frame size is not load-bearing
//!         for downstream consumers, only `samples × 2 == data.len()`
//!         and `sample_rate == 8000` are. Multiple `FillComplexBuffer`
//!         calls per input packet are normal.
//!
//! AMR-NB on AudioToolbox is **decode-only** — AT exposes
//! `kAudioFormatAMR` (`'samr'`) as a decompression target but does not
//! ship a paired encoder. There is no magic cookie: the per-packet TOC
//! byte carries the active mode and AT switches mid-stream as the
...
//! input-queue + one-packet-of-slack pattern as the iLBC decoder so
//! the converter never sees "0 packets" mid-stream, which would put
...
///
/// Holds a FIFO of compressed AMR-NB packets. The callback pops the
...
/// AudioConverter-backed AMR-NB decoder.
pub struct AmrNbAtDecoder {
    codec_id: CodecId,
...
// across threads.
unsafe impl Send for AmrNbAtDecoder {}
...
impl AmrNbAtDecoder {
    fn new(params: &CodecParameters) -> Result<Self> {
        ...
        if let Some(sr) = params.sample_rate {
            if sr != 8_000 {
                return Err(Error::unsupported(format!(
                    "AMR-NB decoder: sample_rate must be 8000 (got {sr})"
                )));
            }
        }
        if let Some(ch) = params.channels {
            if ch != 1 {
                return Err(Error::unsupported(format!(
                    "AMR-NB decoder: channels must be 1 (got {ch})"
                )));
            }
        }

        let in_asbd = AudioStreamBasicDescription::amr_nb();
    ...
    /// Validate an AMR-NB storage-format packet against its TOC byte
    ...
    fn decode_packet(&mut self, data: &[u8]) -> Result<()> {
        ...
        let ft = FrameType::from_toc(toc).ok_or_else(|| {
            Error::invalid(format!(
                "AMR-NB: TOC byte 0x{toc:02x} encodes a reserved frame type (FT={})",
                (toc >> 3) & 0x0F
            ))
        })?;
        ...
        if data.len() != expected {
            return Err(Error::invalid(format!(
                "AMR-NB: {ft:?} packet length {} doesn't match storage-format size {expected}",
                data.len()
            )));
        }
...
impl Decoder for AmrNbAtDecoder {
...
/// Input callback — supplies one compressed AMR-NB packet per call
...
unsafe extern "C" fn amr_input_callback(
    ...
    ctx.packet_desc = AudioStreamPacketDescription {
        start_offset: 0,
        variable_frames_in_packet: amr::FRAMES_PER_PACKET,
...
pub fn make_decoder(params: &CodecParameters) -> Result<Box<dyn Decoder>> {
    Ok(Box::new(AmrNbAtDecoder::new(params)?))
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::amr::{make_toc, FrameType};
    use oxideav_core::{CodecId, CodecParameters};

    fn params_amr() -> CodecParameters {
    ...
    fn make_decoder_succeeds() {
        let r = make_decoder(&params_amr());
        assert!(r.is_ok(), "AMR-NB make_decoder failed: {:?}", r.err());
    }

    #[test]
    fn make_decoder_rejects_bad_sample_rate() {
        let mut p = params_amr();
        p.sample_rate = Some(16_000);
        let r = make_decoder(&p);
        assert!(r.is_err(), "AMR-NB must reject non-8 kHz");
    }
    ...
    fn make_decoder_rejects_stereo() {
        ...
        p.channels = Some(2);
        let r = make_decoder(&p);
        assert!(r.is_err(), "AMR-NB must reject stereo");
    }

    #[test]
    fn send_packet_rejects_reserved_frame_type() {
        let mut dec = AmrNbAtDecoder::new(&params_amr()).expect("decoder construct");
        // FT = 9 is reserved; construct the TOC byte directly.
        let bad_toc = (9u8 << 3) | 0b100;
        let pkt = Packet::new(0, TimeBase::new(1, 8_000), vec![bad_toc; 13]);
        let r = dec.send_packet(&pkt);
        assert!(r.is_err(), "must reject reserved FT");
    }

    #[test]
    fn send_packet_rejects_size_mismatch() {
        let mut dec = AmrNbAtDecoder::new(&params_amr()).expect("decoder construct");
        // Valid MR475 TOC but wrong byte count (12 instead of 13).
        let toc = make_toc(FrameType::Mr475);
        let pkt = Packet::new(0, TimeBase::new(1, 8_000), vec![toc; 12]);
        let r = dec.send_packet(&pkt);
    ...
    fn send_packet_accepts_no_data_frame() {
        // NO_DATA is a 1-byte packet (TOC byte only). Useful to prove
        // the queue + size validation path is correct even when no PCM
        // can possibly be produced.
        let mut dec = AmrNbAtDecoder::new(&params_amr()).expect("decoder construct");
        let toc = make_toc(FrameType::NoData);
        let pkt = Packet::new(0, TimeBase::new(1, 8_000), vec![toc]);
        let r = dec.send_packet(&pkt);
    ...
    fn send_packet_accepts_sid_frame() {
        ...
        let pkt = Packet::new(0, TimeBase::new(1, 8_000), vec![toc; 6]);
        let r = dec.send_packet(&pkt);
    ...
    fn send_packet_accepts_speech_mode_packets() {
        // Each speech mode should pass the size + TOC validation when
        // handed a syntactically-correct zero-payload packet.
        let mut dec = AmrNbAtDecoder::new(&params_amr()).expect("decoder construct");
        let modes = [
            FrameType::Mr475,
            FrameType::Mr515,
        ...
        for ft in modes {
            ...
            let pkt = Packet::new(0, TimeBase::new(1, 8_000), buf);
            let r = dec.send_packet(&pkt);
