...
| MP3       | yes     | n/a     | yes (MPEG-1/2/2.5 Audio Layer III via `kAudioFormatMPEGLayer3` = `'.mp3'`; AT exposes MP1 / MP2 / MP3 decode-only — no paired encoder for any layer) |
...
Round 9 MP3 (MPEG-1/2/2.5 Audio Layer III, ISO/IEC 11172-3 / 13818-3) **decode**: the dominant consumer lossy-audio format, 32 kHz / 44.1 kHz / 48 kHz (MPEG-1), 16 / 22.05 / 24 kHz (MPEG-2 LSF), 8 / 11.025 / 12 kHz (Fraunhofer MPEG-2.5), 1–2 channels, CBR / VBR / ABR. AudioToolbox exposes `kAudioFormatMPEGLayer3` (`'.mp3'`) as a decode-only target — there is no paired AT encoder for any MPEG audio layer (`'.mp1'` / `'.mp2'` constants are also added to `sys.rs` for completeness even though only the MP3 codec id is registered, since the same decoder handles all three through automatic layer sniffing). The new `src/mp3.rs` parses the 32-bit MPEG audio frame header (sync + version + layer + bitrate-index + sample-rate-index + padding + channel-mode), with per-(version × layer) bitrate / sample-rate / samples-per-frame / frame-length lookup tables matching ISO/IEC 11172-3 §2.4.2.3 and 13818-3 §2.4.2.3. `src/mp3_decoder.rs` lazy-configures the converter from the first frame's header (sample rate, channel count, layer) and rejects mid-stream layer / rate / channel-mode switches with typed errors before AT can refuse them. Validates each subsequent frame's declared length matches the packet bytes handed in. Persistent input-queue + one-packet-of-slack lookahead pattern matching AAC LC / HE / iLBC / AMR. Tags claimed: FourCC `'.mp3'` (MOV `.mp3` sample entries), ISO/IEC 14496-1 OTI `0x6B` (MPEG-1/2 Audio in `mp4a` sample entries that opt into the legacy MP3 OTI), Matroska `A_MPEG/L3`, and wFormatTag `0x0055` (Microsoft's RIFF/WAVE tag for MP3-in-WAV). Round-trip on the staged 33-frame MPEG-1 LIII 128 kbit/s 44.1 kHz stereo fixture (`docs/audio/mp3/fixtures/layer3-stereo-44100-128kbps/`): 38,016 PCM samples per channel produced (33 × 1152, bit-exact frame count), per-channel **SNR ≥ 89 dB** against the staged reference WAV after the priming-silence alignment search — well into transparent-decode territory, proves the AT MP3 path matches the staged reference's IMDCT output to within rounding noise.
