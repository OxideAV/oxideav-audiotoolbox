//! Bit-exact round-trip test for the AudioToolbox ALAC bridge.
//!
...
//! encodes it through `AlacAtEncoder`, then forwards the resulting raw
//! ALAC packets + encoder-vended magic cookie into `AlacAtDecoder`, and
//! verifies the recovered PCM matches the input **bit-for-bit** after
//! accounting for encoder priming silence at the head.
//!
...
/// Build a deterministic interleaved S16 PCM signal: 440 Hz sine
/// mixed with a small LCG-driven pseudo-random component so byte-exact
/// recovery is a meaningful assertion (a sine alone would survive
/// many *non*-bit-exact codecs too).
fn gen_signal_s16(n_frames: usize) -> Vec<i16> {
    ...
    for i in 0..n_frames {
        ...
        // 24000 ≈ 0.73 of full-scale → leaves headroom for the noise term.
        let s_main = (sine * 24_000.0) as i32;
        // Per-sample 12-bit noise so we exercise the entropy coder.
        lcg = lcg.wrapping_mul(1_664_525).wrapping_add(1_013_904_223);
        let noise = ((lcg >> 16) as i16) >> 4; // ±2048
        let s = (s_main + noise as i32).clamp(i16::MIN as i32, i16::MAX as i32) as i16;
        // Stereo: identical on both channels keeps the test simple
        // while still forcing the encoder to handle 2-channel decorrelation.
        for _ in 0..CH {
            out.push(s);
        }
    }
...
fn alac_roundtrip_bit_exact() {
    ...
    let mut enc = alac_encoder::make_encoder(&enc_params).expect("make_encoder");

    // Feed PCM in arbitrary chunks (not aligned to packet boundary) to
    // exercise the encoder's internal staging buffer.
    ...
    // Grab the encoder-vended magic cookie before dropping `enc`.
    ...
    eprintln!(
        "ALAC roundtrip: ref={} samples, dec={} samples, {} packets",
        ref_samples_i16.len(),
        decoded_samples.len(),
        alac_packets.len()
    );
    ...
    //
    // AT's ALAC encoder may prepend a small amount of priming silence
    ...
    eprintln!(
        "ALAC priming search: best offset = {best_offset} samples, \
    ...
    // assertion for a lossless codec.
    assert!(
        best_match >= 4096,
        "ALAC roundtrip not bit-exact: best contiguous match = {best_match} samples at offset {best_offset}"
    );

    // Now extend the bit-exact comparison from `best_offset` and
    // demand it survives across multiple ALAC packets.
    ...
    eprintln!("ALAC bit-exact run from priming offset {best_offset}: {bit_exact_run} samples",);
    // We expect at least 3 full packets (3 × 4096 × 2 chans = 24576 samples)
    // to survive — proves the codec isn't just lossy with a lucky
    // prefix.
    assert!(
        bit_exact_run >= 3 * PACKET_FRAMES * CH as usize,
        "ALAC roundtrip lost bit-exactness after {bit_exact_run} samples \
         (need ≥ {})",
